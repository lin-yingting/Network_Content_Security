...
model = xgb.XGBRegressor(max_depth=6,learning_rate=0.05,n_estimators=30)
...
msetrain=mean_squared_error(y_train,predict_train)
...
model2 = xgb.XGBRegressor(max_depth=6,learning_rate=0.05,n_estimators=30)
# model = xgb.XGBRegressor(max_depth=6,learning_rate=0.05,n_estimators=100)
model2.fit(x_train2,y_train2)
predict_test2 = model2.predict(x_test2)
predict_train2 = model2.predict(x_train2)

dataframe = pd.DataFrame({'xgb预测值':predict_test2,'xgb真实值':y_test2})
dataframe.to_csv(r'D:\\pythonProject_al\\data\\xgb_predicted.csv', index=False)


# msetest2=mean_squared_error(y_test2,predict_test2)
# msetrain2=mean_squared_error(y_train2,predict_train2)
...
# page.render("xgb_all.html")

#生成布局完成的网页
# Page.save_resize_html("xgb_all.html", cfg_file="D:\\pythonProject_al\\chart_config3.json", dest="xgb_all2.html")
...
# #####################################
# #用网格法判断XGBoost的n_estimators较佳取值
# ####################################
#
...
# gsearch1 = GridSearchCV(estimator=xgb.XGBRegressor(max_depth=6,learning_rate=0.05),param_grid=param_test1,cv=10,scoring=r2_secret_mse)#不同超参数十折交叉验证
